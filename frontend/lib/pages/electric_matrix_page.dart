...
/// ElectricMatrixPage - Halaman Form Matriks Electric 2 (dengan pilihan gerbong & fase)
class ElectricMatrixPage extends StatefulWidget {
  const ElectricMatrixPage({Key? key}) : super(key: key);

  @override
  State<ElectricMatrixPage> createState() => _ElectricMatrixPageState();
}

class _ElectricMatrixPageState extends State<ElectricMatrixPage> {
  final List<int> _carOptions = List.generate(10, (index) => index + 1);
  final List<String> _phaseOptions = ['Fase 1', 'Fase 2'];
    ...
    showDialog(
      ...
      builder:
          (_) => AlertDialog(
            ...
            content: Text(
              'Form Electric - Gerbong $gerbong ($fase) berhasil disimpan.',
            ),
            actions: [
              TextButton(
                onPressed: () {
              ...
              onChanged: (value) {
                setState(() {
                  ...
                  if (_selectedCar != null && _selectedPhase != null) {
                    _futureMatrix = DummyApiService.fetchElectricMatrixItems(
                      _selectedCar!,
                      _selectedPhase!,
                    );
                  } else {
